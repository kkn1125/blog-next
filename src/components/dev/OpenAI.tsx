...
import { SECRET_KEY } from "../../util/global";
import { Base64 } from "../../util/tool";
import "./OpenAI.module.css";

const md = markdown();
md.options.highlight = (code: string | HighlightOptions, lang: string) => {
    ...
    Object.entries(v)
      ...
      .flatMap(([k2, v2]) =>
        Object.entries(v2)
          ...
          .flatMap(([k3, v3]) =>
            (v3 as any).map(
              (vv: { tag: string; todo: string; time: string }, i: number) =>
                k +
                "년" +
                (Number(k2) + 1) +
                "월" +
                ...
                "일 " +
                `${
                  daysName[new Date(k as any, k2 as any, k3 as any).getDay()]
                }요일` +
                (i + 1) +
                "번 일정은 tag가" +
                vv.tag +
...
function OpenAI({ open, onClose }: any) {
  let staticMessages: any[] = [];
  ...
  function handleMouseDownCloseModal(e: { target: any }) {
  ...
  function intervalOutput(datas: any[]) {
    let tempRole = "";
    let tempContent = "";
