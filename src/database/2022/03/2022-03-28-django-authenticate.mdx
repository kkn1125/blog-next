...
modified: 2023-09-05 13:16:08 +0000
...
![pgAdmin 속성](/assets/images/post/django/authentication/auth01.png)
...
{% load static %} {% load customs %} {% set request.path as path %}
<!DOCTYPE html>
<html lang="ko">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
          ...
          <ul class="navbar-nav me-auto mb-lg-0">
            ...
            <li class="nav-item">
              <form action="{% url 'account:signout' %}" method="post">
                {% csrf_token %}
                <button class="nav-link border-0 bg-transparent" href="">
                  Sign out
                </button>
              </form>
            </li>
            ...
            <li class="nav-item">
              <a
                class="nav-link{{path|is_active:'signin'}}"
            ...
            <li class="nav-item">
              <a
                class="nav-link{{path|is_active:'signup'}}"
    ...
    <div class="container">
      <div class="my-5">{% block content %}{% endblock %}</div>
    </div>
    <script
...
{% extends './layout.html' %} {% block content %} 홈입니다. {% endblock %}
=============================
...
![메인](/assets/images/post/django/authentication/auth02.png)

로그인과 회원가입이 있습니다. 로그인 되면 로그아웃과 사용자 명이 나타나게 됩니다.

![회원가입 폼](/assets/images/post/django/authentication/auth03.png)

지저분하지만 필요에 따라 다듬으면 됩니다. `AbstractUser`로 확장할 때 다듬어 보려합니다!

![로그인 폼](/assets/images/post/django/authentication/auth05.png)

로그인하게 되면 메인으로 이동하고,

![로그인](/assets/images/post/django/authentication/auth06.png)
