...
modified: 2023-09-05 13:15:19 +0000
...
<div class='btn-bundle'>
  <button class='btn btn-info' target='origin' group='one'>
    templatetags
  </button>
  <button class='btn btn-info' target='change' group='one'>
    my_tags
  </button>
</div>
...
<div page='change' group='one'>
  templatetags가 아닌 다른 이름으로 변경할 경우 settings.py에서 templates에
  libraries를 추가해줘야 한다.
</div>
...
<div class='btn-bundle'>
  <button class='btn btn-info' target='first' group='two'>
    for문
  </button>
  <button class='btn btn-info' target='second' group='two'>
    단독
  </button>
</div>
...
{% load customs %} ...
<div class="container">
  {% with todo.tags|split:',' as tags%} {% for tag in tags %} {{tag}}<br />
  {% endfor %} {% endWith %}
</div>
...
```
