...
modified: 2023-09-05 13:13:56 +0000
...
```html
{% extends "path/index.html" %} 해당 경로에 block을 넣는다. {% block title %} {{
info.title }} {% endblock %} {% block content %} {% for post in posts %}
<h1>{{ post.title }}</h1>
...
[Tigercow.Door님 :: 장고(django) #3\_ django에서 postgreSQL 사용하기](https://doorbw.tistory.com/183)
