...
modified: 2023-09-05 13:13:07 +0000
...
function completionCountNotification(target) {
  return (remaining.innerHTML = `${target.length}개 중 ${
    target.filter((t) => t.done).length
  }개 완료 👍`);
}

function writeOnDisplay(target) {
  return (display.innerHTML = target
    .map(
      (t, i) =>
        `<li>
         <span class="${t.done ? "check" : ""}">${i + 1}. ${t.text || ""}</span>
         <button ${t.done ? 'class="checked"' : ""} onclick="checkHandler(${
          t.id
        })">Check</button>
...
![결과](/assets/images/post/proxy/proxy01.png)

## Scss로 조금 꾸민 후

![꾸민 후](/assets/images/post/proxy/proxy02.png)
