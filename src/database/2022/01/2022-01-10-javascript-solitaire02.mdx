...
modified: 2023-09-05 13:06:21 +0000
...
function Controller() {
  ...
  this.cardCollect = function (ev) {
    ...
    if (
      !target.parentNode.classList.contains("stacking") ||
      target.parentNode.lastElementChild != target
    )
      return; // 부모가 stacking 클래스가 아니거나 마지막 요소가 아니라면
    // 함수를 멈춥니다.
  ...
  this.cardDraw = function (ev) {
    ...
    if (
      ...
      !target.parentNode.classList.contains("stock")
    )
      return;
...
function Model() {
  ...
  this.generateCardSuits = function ({ suits, list }) {
    [...suits].forEach((type) => {
  ...
  this.validNextCard = function (card) {
    ...
    if (card.deno == 1) return true;
      ...
      card.deno ==
      cardStack[parts.card.suits.indexOf(card.$suit)]?.slice(-1)?.pop()?.deno +
        1
    )
      return true;
  };
  ...
  this.findCard = function (card) {
    ...
    return []
      ...
      .filter((c) => c.id == card.dataset.cardId)
      .pop();
  };
}
```
...
function View() {
  ...
  this.renderFrames = function () {
    ...
    document.body.insertAdjacentHTML(
      "afterbegin",
      parts.template.frame.render()
    ); // ++
    document.body.insertAdjacentHTML(
      "afterbegin",
      parts.template.option.render()
    ); // ++
  ...
  this.cardDraw = function (cardStock) {
    ...
    staged.reverse().forEach((s, idx) => {
      elStock
        .querySelector(".stacking")
        .insertAdjacentHTML("beforeend", parts.card.render(s));
    });

    [...elStock.querySelector(".stacking").children]
      .slice(1)
      .slice(-3)
      .forEach((el, idx) => {
  ...
  this.cardCollect = function (cardStack) {
    ...
    cardStack.forEach((stack) => {
      stack.forEach((s, idx) => {
        [...elStack.children][
          parts.card.suits.indexOf(s.$suit)
        ].insertAdjacentHTML("beforeend", parts.card.render(s));
      });
    });
  };
  ...
  this.cardPlaying = function (cardPlay) {
    ...
    for (let col in cardPlay) {
      for (let row in cardPlay[col]) {
        [...elColumns.children][col].insertAdjacentHTML(
          "beforeend",
          parts.card.render(cardPlay[col][row])
        );
        [...elColumns.children][col].lastElementChild.style.top =
          row * 20 + "px";
      }
    }
  };
...
![현재까지 결과물](/assets/images/post/solitaire/solitaire03.png)
        ...
        card: {
          ...
          render(card) {
            ...
            return `
                            ...
                            style="background-image: url('./src/img/${
                              card.imgNum
                            }_of_${card.imgSuit}.png')">
                            </div>
                        `; // ++ img 가 추가 되었습니다.
          },
        },
