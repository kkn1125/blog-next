...
modified: 2023-09-05 13:06:30 +0000
...
function Model() {
  ...
  this.cardMove = function (elCard) {
    ...
    if (Model.selectBundle.length == 0 && !elCard.classList.contains("front"))
      return;
    ...
    if (Model.selectBundle.length == 2) {
      ...
      if (this.isStackable([first.card, second.card])) {
        ...
        if (first.card.$parent[0] instanceof Array) {
          ...
          const last = first.card.$parent
            .splice(first.card.$parent.indexOf(first.card), 1)
            .pop();
          last.$parent = second.card.$parent;
          temp.push(last);
        }
  ...
  this.cardMoveToEmpty = function (card, col, idx) {
    ...
    if (card.card.$parent[0] instanceof Array) {
      ...
      const last = card.card.$parent
        .splice(card.card.$parent.indexOf(card.card), 1)
        .pop();
      last.$parent = cardPlaying;
      temp.push(last);
    }
  ...
  this.isStackable = function ([first, second]) {
    if (
      parseInt(first.deno) + 1 == parseInt(second.deno) &&
      this.isCrossable(first.$suit, second.$suit)
    ) {
      return true;
    } else {
      return false;
  ...
  this.isCrossable = function (first, second) {
    ...
    return (
      (isLeft(first) && isRight(second)) || (isRight(first) && isLeft(second))
    );
  };
...
![현재까지 결과물](/assets/images/post/solitaire/solitaire04.png)
