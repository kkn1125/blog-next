...
![대구 공연 정보](/assets/images/post/publicData/public01.png)
...
![JSON내용](/assets/images/post/publicData/public02.png)
...
![파싱후](/assets/images/post/publicData/public03.png)
  ...
  this.template = function () {
    return ` 
        <div id="card" class="bg-light p-4 rounded-3 border border-primary mb-3">
            <h3 class="text-truncate" title="${this.subject}">${this.num}-${
      this.subject
    }</h3>
            <div class="my-3">${
              this.content == null ? "내용이없습니다" : this.content
            }</div>
            <div>${this.place}</div>
            <div>
...
![리스트](/assets/images/post/publicData/public04.png)
  ...
  if (i > total / 2 - 5 && i < total / 2 + 5) {
    ...
    paging.innerHTML += `
        <li class="page-item${i + 1 == page ? " fw-bold" : ""}">
            <a href="${origin + path}?page=${i + 1}" class="page-link">${
      i + 1
    }</a>
        </li>
        `;
...
<li class="page-item">
    <a href="${origin + path}?page=${
    page == total ? page : parseInt(page) + 1
  }" class="page-link">&gt;&gt;</a>
</li>
`;
```

![완성](/assets/images/post/publicData/public-move.gif)
