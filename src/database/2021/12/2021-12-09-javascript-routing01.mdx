...
modified: 2023-09-05 12:43:38 +0000
...
function handleAnchor(ev) {
  ...
  if (name == "A") {
    ...
    if (validHTTP(attrs["href"].value))
      window.open(href); // 외부 링크 새창으로 열기
    else {
      if (hash !== "") {
...
this.init = function (components) {
  ...
  app = document.body.insertAdjacentElement(
    "beforeend",
...
this.renderPublicOptions = function () {
  Object.keys(parts).forEach((part) =>
    app.insertAdjacentHTML("beforeend", moduler.parts[part].render())
  );
};
```
