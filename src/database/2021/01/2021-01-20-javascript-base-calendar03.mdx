...
function calendar_make(a, b) {
  ...
  for (var i = 1; i <= last_date; i++) {
    ...
    cell.addEventListener("click", function (self) {
      document
        .querySelector("#word_add")
        .setAttribute(
          "onclick",
          "javascript:words_add(" + self.target.id + ")"
        );
    });
    cell.innerHTML = i;
  }
...
<figure className='text-center'>
  <span className='w-inline-block'>
    <img src='/assets/images/post/back/back03.png' alt='sample' title='sample' />
  ...
  <body>
    <div id="setcal">
      <div>
              ...
              <tr>
                <td>
                  <input
                    type="button"
                    value="&lt&lt"
                    onclick="javascript:before_after(this)"
                  />
                </td>
        ...
        <button id="go_today" onclick="javascript:calendar_today()">
          <span>Today</span>
        </button>
      </div>
      <div>
