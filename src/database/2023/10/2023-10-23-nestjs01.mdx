---
slug: "/nestjs01/"
date: 2023-10-23 18:37:15 +0000
modified: 2023-10-23 18:37:15 +0000
title: "[NESTJS] Nest.js 프로젝트 초기 설정하기"
author: Kimson
categories: [nestjs]
image: /assets/images/post/covers/TIL-nest2.png
tags: [node, nest, framework, basic, til]
description: "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Officiis quia saepe ullam assumenda perferendis aliquid ratione? Inventore aperiam optio soluta, iste temporibus dolor dicta! Amet earum doloribus illo reprehenderit magni."
featured: true
rating: 4.5
profile: false
published: true
...
제일 만만한 게시판을 제작하기 위해 board 리소스를 생성한다. 아주 놀랍게도 컨트롤러와 서비스에 이어서 엔티티와 DTO를 무려 생성, 수정 포맷에 맞게 초기화 해준다.
...
11라인의 `app.useLogger`는 네스트가 출력하는 로거를 커스텀로거로 지정하여 사용하도록 한다. `main.ts`는 단지 기본 로거를 커스텀로거로 사용하는 설정을 하는 것이지 모든 모듈에 `import`하는 로거 모듈과는 상관없다. 각 모듈에서 로거를 사용한다면 `import`가 필수이다.
