...
modified: 2023-09-05 13:22:16 +0000
...
2. _npm_ - 9.6.1
3. _fastify_ - 4.15.0
4. _node-schedule_ - 2.1.1
5. _pino-pretty_ - 10.0.0 (선택사항)
...
export default class CacheManager {
  ...
  initialize(key) {
    Object.entries(this.store).forEach(([k, v]) => {
      if (k.match(key)) {
        console.log(
          `[❌ initialize match store]: cache in ${key} => delete ${k}`
        );
        delete this.store[k];
      }
    });
...
![크론시간으로 정책 실행](https://user-images.githubusercontent.com/113876485/230785226-c8990fc0-1462-41e0-86e3-023b9e0c701f.png)

크론시간으로 5초마다 정책을 실행하는 모습입니다.

![첫 번째 요청에 대한 응답 딜레이](https://user-images.githubusercontent.com/113876485/230784796-815400dd-602d-4fc8-95f8-38aaaf7b68a5.png)

처음 요청하게 되면 500ms이상의 지연시간이 생깁니다.

![첫 요청 후 min, max 비교 (LFU 정책)](https://user-images.githubusercontent.com/113876485/230785570-793d2b40-f91d-440a-9338-748fb6fcad29.png)

첫 번째 요청에 대한 로그 확인입니다. 첫 요청이기 때문에 min, max 캐시 모두 동일합니다.

![두 번째 요청에 대한 응답 딜레이](https://user-images.githubusercontent.com/113876485/230785303-a7a0a05d-8a81-401d-8750-7119fdffb02c.png)

다시 요청하게 되면 캐시데이터가 저장되어 있기 때문에 데이터베이스를 조회하지 않고 저장된 캐시 데이터를 바로 응답합니다.

![사용자 데이터 추가](https://user-images.githubusercontent.com/113876485/230785265-b995345f-09e1-407b-a175-a76c0a286b3c.png)

이때 사용자 데이터가 추가/수정/삭제 된다면 조회하는 데이터를 업데이트 되야하므로 초기화 작업을 거쳐 사용자 API 캐시 데이터 모두 교체 준비를 합니다.

![사용자 추가로 인해 캐시 데이터 초기화](https://user-images.githubusercontent.com/113876485/230785339-4b8310d1-57e1-4542-b671-d4a1910f9b38.png)

사용자가 추가되었기 떄문에 사용자 조회하게 되면 응답 시간이 다시 길어집니다.

![다시 사용자 조회](https://user-images.githubusercontent.com/113876485/230785389-4b79099c-bed8-4cb2-9ff1-246f85e3bd6c.png)

이후 다시 요청하면 캐시에 저장된 사용자 데이터를 곧바로 응답하게 됩니다.

![전제조회 4, 단일 조회 2](https://user-images.githubusercontent.com/113876485/230786078-da8e6732-a4cf-4342-ad78-2ef5cea701ad.png)
...
![LFU 정책으로 적은 요청 횟수의 캐시 삭제](https://user-images.githubusercontent.com/113876485/230786181-017f70bc-d8f6-4c52-b7e7-a4fcf5ab1df0.png)

이후 정책에 의해 해당 캐시 데이터가 삭제 되었기 때문에 전체 사용자 조회 데이터만 남습니다.

![마지막 남은 캐시 데이터](https://user-images.githubusercontent.com/113876485/230786299-fa69283a-4fd8-4d7c-b9e0-923a6d760988.png)
